...
export const RegisterSchema = z.object({
    uusername: z.string({required_error: "username is required!", invalid_type_error: "username must be a string"}).min(3, { message: "username must be more then just 3 letters!"}).max(15, { message: "username must be less then 15 letters!"}),
    ...
    address: z.object({
        street: z.string({required_error: "street is required!", invalid_type_error: "street must be a string"}).min(3, { message: "street must be more then just 3 letters!"}).max(15, { message: "street must be less then 15 letters!"}),
        city: z.string({required_error: "city is required!", invalid_type_error: "city must be a string"}).min(3, { message: "city must be more then just 3 letters!"}).max(15, { message: "city must be less then 15 letters!"}),
        houseNumber: z.string({required_error: "houseNumber is required!", invalid_type_error: "houseNumber must be a string"}).min(3, { message: "houseNumber must be more then just 3 letters!"}).max(15, { message: "houseNumber must be less then 15 letters!"}),
    })
